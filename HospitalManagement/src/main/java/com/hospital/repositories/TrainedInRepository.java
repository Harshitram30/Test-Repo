...
public interface TrainedInRepository extends JpaRepository<TrainedIn, TrainedInId>{
	...
	//@Query("select p from Procedure p JOIN p.trainedIn t where t.certificationexpires BETWEEN :CURRENT_DATE AND :oneMonthFromNow")
	//List<Procedure> findByExpirationDate(@Param("Current Date")Date start, @Param("oneMonthFromNow")Date end);
	
	@Query("SELECT t FROM TrainedIn t WHERE t.physician = :physicianid AND t.treatment= :procedureid")
    TrainedIn findByPhysicianAndProcedure(@Param("physicianid") Integer physicianid, @Param("procedureid") Integer procedureid);
